...
function getCurrent5mSlug(ticker: 'btc' | 'eth' | 'sol' | 'bnb'): string {
    const now = new Date();
...
export async function getCurrentMarket(ticker: 'btc' | 'eth' | 'sol' | 'bnb') {
...
export async function getCurrentBNBMarket() {
    return getCurrentMarket('bnb');
}
